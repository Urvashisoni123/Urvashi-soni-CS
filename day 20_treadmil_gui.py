...
values=['Select','Product','Age','Gender']
col3.set(values[0])
ttk.Label(app,text = 'Column 1 ').place(x=100,y=150)
...
def show():
    ...
    if 'col1' in g:
        if column1 == 'Select':
            result.set('column1 will be selected')
            return

    if 'col2' in g:
        if column2 == 'Select':
            result.set('column2 will be selected')
            return

    if 'col3' in g:
        if column3 == 'Select':
            result.set('column3 will be selected')
            return        
    ...
    img = ImageTk.PhotoImage(
        Image.open('graph.png').resize((200,100))
    )
